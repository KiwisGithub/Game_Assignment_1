...
namespace _2019_Level2_Dodge
{
    class Stars2
    {
        ...
        public Stars2(Rectangle spaceRec, int missileRotate)
        {

            int[] quoteDoge = new int[] { 4, 2, 1, 3, 1, 2, 3 };
            Random rnd = new Random();
            int r = rnd.Next(7);
            int str = quoteDoge[r];

            int[] quoteDoge2 = new int[] { 1, 2,2, 3 };
            ...
            xSpeed = 3 * (Math.Cos((missileRotate - 90) * Math.PI / 180));
            ySpeed = 3 * (Math.Sin((missileRotate + 90) * Math.PI / 180));
