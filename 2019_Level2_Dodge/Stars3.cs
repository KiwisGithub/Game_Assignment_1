...
namespace _2019_Level2_Dodge
{
    class Stars3
    {
        ...
        public Stars3(Rectangle spaceRec, int missileRotate)
        {

            int[] quoteDoge = new int[] { 3, 1, 1, 2, 1, 1, 2 };
            Random rnd = new Random();
            int r = rnd.Next(7);
            int str = quoteDoge[r];

            int[] quoteDoge2 = new int[] { 1, 1, 2 };
            ...
            xSpeed = 1 * (Math.Cos((missileRotate - 90) * Math.PI / 180));
            ySpeed = 1 * (Math.Sin((missileRotate + 90) * Math.PI / 180));
